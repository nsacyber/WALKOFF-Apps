...
@action
def get_children(pid, with_grandchildren=False):
    process = psutil.Process(pid)
...
@action
def get_file_argument_to_script_command_line(pid):
    process = psutil.Process(pid)
    command_line = process.cmdline()
    if not command_line:
        return 'Command line not found', 'CommandLineNotFound'
...
def get_all_connections(self, connection_type='all', only_not_localhost=False, only_with_pids=False):
    return self.all_connections_as_json(
...
@action
def watch_all_connections(watch_time=10, connection_type='all', only_not_localhost=False, only_with_pids=False):
...
@action
def blame_processes_for_ip_connection(ip, connection_type='all', watch_time=10):
    start = datetime.utcnow()
    if not connection_type:
        connection_type = 'all'
    results = set()
...
@action
def get_current_working_directory(pid):
    process = psutil.Process(pid)
    return process.cwd()
...
@action
def copy_and_reverse_exe(pid, filename=None):
    process = psutil.Process(pid)
    exe = process.exe()
    if not exe:
