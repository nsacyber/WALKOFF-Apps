...
class Main(App):
    ...
    def create_accounts(self, total_nodes):
        ...
        res &= self._run_script(["step1-create-accounts.sh", self.totalNodes])
        return res
    ...
    def set_up_network(self):
        ...
        res &= self._run_script(["step4-connect-miners.sh", self.totalNodes])
        res &= self._run_script(["step5-deploy-contract.sh", self.totalNodes])
        return res

    @action
    def submit_greeting(self, greeting):
        self.greeting = greeting
        return self._run_script(["step6-submit-greeting.sh", self.greeting])
